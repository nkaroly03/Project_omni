...
using Op_code = Compile.Compiler.Op_code;
    ...
    public static Value run(ReadOnlySpan<byte> bytecode, Value argv){
        ...
        for (int pc = 0; pc < bytecode.Length;){
            switch ((Op_code)bytecode[pc++]){
                ...
                case Op_code.TO_BOOL:  stack[^1] = new(stack[^1].to_bool());   break;
                case Op_code.TO_CHAR:  stack[^1] = new(stack[^1].to_char());   break;
                case Op_code.TO_INT:   stack[^1] = new(stack[^1].to_int());    break;
                case Op_code.TO_FLOAT: stack[^1] = new(stack[^1].to_float());  break;
                ...
                case Op_code.ALLOC_ARRAY:
                    ...
                    stack[^1] = (Op_code)bytecode[pc++] switch{
                        ...
                        Op_code.TO_CHAR  => new(new char [alloc_size]),
                        Op_code.TO_INT   => new(new int  [alloc_size]),
                        Op_code.TO_FLOAT => new(new float[alloc_size]),
                        Op_code.TO_STR   => new(((Func<StringBuilder[]>)(() => {
                ...
                case Op_code.CMP_EQ:  stack[^2] = new(stack[^2] == stack[^1]); stack.pop(); break;
                case Op_code.CMP_NEQ: stack[^2] = new(stack[^2] != stack[^1]); stack.pop(); break;
                case Op_code.CMP_LE:  stack[^2] = new(stack[^2] <  stack[^1]); stack.pop(); break;
                case Op_code.CMP_LEQ: stack[^2] = new(stack[^2] <= stack[^1]); stack.pop(); break;
                case Op_code.CMP_GE:  stack[^2] = new(stack[^2] >  stack[^1]); stack.pop(); break;
                case Op_code.CMP_GEQ: stack[^2] = new(stack[^2] >= stack[^1]); stack.pop(); break;

                case Op_code.ADD:  stack[^2]  += stack[^1]; stack.pop(); break;
                case Op_code.SUB:  stack[^2]  -= stack[^1]; stack.pop(); break;
                case Op_code.MUL:  stack[^2]  *= stack[^1]; stack.pop(); break;
                case Op_code.DIV:  stack[^2]  /= stack[^1]; stack.pop(); break;
                case Op_code.MOD:  stack[^2]  %= stack[^1]; stack.pop(); break;
                case Op_code.SHL:  stack[^2] <<= stack[^1]; stack.pop(); break;
                case Op_code.SHR:  stack[^2] >>= stack[^1]; stack.pop(); break;
                case Op_code.BAND: stack[^2]  &= stack[^1]; stack.pop(); break;
                case Op_code.BOR:  stack[^2]  |= stack[^1]; stack.pop(); break;
                case Op_code.XOR:  stack[^2]  ^= stack[^1]; stack.pop(); break;
